...
export FFV_HOME=/home/hp120184/k02576/scratch/FFVC_LPT/
export TMP_LDFLAGS=
...
../configure --prefix=${FFV_HOME}/TextParser \
             --with-comp=FJ \
             CXX=$TMP_CXX \
             CXXFLAGS="-Kfast,ocl,preex,simd=2,uxsimd,array_private,parallel,openmp,optmsg=2 -V -x0" --host=sparc64-unknown-linux-gnu
make
if [ $? -ne 0 ]; then
  echo "make error!"
  exit
...
../configure --prefix=${FFV_HOME}/PMlib \
             ...
             CXXFLAGS="-Kfast,ocl,preex,simd=2,uxsimd,array_private,parallel,openmp,optmsg=2 -V -x0" --host=sparc64-unknown-linux-gnu \
             CC=$TMP_CCC \
             CFLAGS="-Kfast,ocl,preex,simd=2,uxsimd,array_private,parallel,openmp,optmsg=2 -V -x0" --host=sparc64-unknown-linux-gnu
make
if [ $? -ne 0 ]; then
  echo "make error!"
  exit
...
../configure --prefix=${FFV_HOME}/CPMlib \
             ...
             --with-f90real=$PRCSN2 \
             CXX=$TMP_CXX \
             CXXFLAGS="-Kfast,ocl,preex,simd=2,uxsimd,array_private,parallel,openmp,optmsg=2 -V -x0" \
             F90=$TMP_F90 \
             F90FLAGS="-Kfast,ocl,preex,simd=2,uxsimd,array_private,parallel,openmp,optmsg=2 -V " --host=sparc64-unknown-linux-gnu \
             LDFLAGS=$TMP_LDFLAGS
make
if [ $? -ne 0 ]; then
  echo "make error!"
...
../configure --prefix=${FFV_HOME}/CDMlib \
             --with-parser=${FFV_HOME}/TextParser \
             F90=$TMP_F90 \
             F90FLAGS="-Kfast,ocl,preex,simd=2,uxsimd,array_private,parallel,openmp,optmsg=2 -V " \
             CXX=$TMP_CXX \
             CXXFLAGS="-Kfast,ocl,preex,simd=2,uxsimd,array_private,parallel,openmp,optmsg=2 -V -x0" --host=sparc64-unknown-linux-gnu
make
if [ $? -ne 0 ]; then
...
pushd ${PDM_LIB}
./install.sh -c FX -p${FFV_HOME} -t${FFV_HOME}/TextParser
if [ $? -ne 0 ]; then
  echo "make error!"
  exit
fi
...
pushd ${LPT_LIB}
autoreconf -i
./configure --prefix=${FFV_HOME}/LPTlib \
            --with-pm=${FFV_HOME}/PMlib \
            ...
            CXX=$TMP_CXX \
            CXXFLAGS="-Kfast,ocl,preex,simd=2,uxsimd,array_private,openmp,optmsg=2 -V -x0" 
make
if [ $? -ne 0 ]; then
  echo "make error!"
  exit
...
../configure --prefix=${FFV_HOME}/FFVC \
             ...
             CCC=$TMP_CCC \
             CFLAGS="-Kfast,ocl,preex,simd=2,uxsimd,array_private,parallel,openmp,optmsg=2 -V -x0" \
             CXX=$TMP_CXX \
             CXXFLAGS="-Kfast,ocl,preex,simd=2,uxsimd,array_private,parallel,openmp,optmsg=2 -V -x0" \
             F90FLAGS="-Kfast,ocl,preex,simd=2,uxsimd,array_private,parallel,openmp,optmsg=2 -V " \
